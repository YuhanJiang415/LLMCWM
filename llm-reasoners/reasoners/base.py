...
class Evaluator():
    ...
    def evaluate(self,
        ...
        for i, example in enumerate(tqdm(self.dataset,
            ...
            correct = self.eval_output(answer, output, check_goal_satisfied(algo_output.tree_state))
            correct_count += correct
            accuracy = correct_count / (i + 1)
            log_str = f'Case #{resume + i + 1}: {correct=}, {output=}, {answer=};'\
                        f'{accuracy=:.3f} ({correct_count}/{i + 1})'
            tqdm.write(log_str)
            wandb.log({"accuracy": accuracy}, step=resume + i + 1)
...
def check_goal_satisfied(node, goal_value: float = 100.0) -> bool:
    """
