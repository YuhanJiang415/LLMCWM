    ...
    def _expand(self, node: MCTSNode):
        ...
        if self.parallel_actions:
            ...
            for action in actions:
                # Time the execution
                # print(f"Tracing action {action} at depth {node.depth} in node {node.state.description}")
                fast_reward, fast_reward_detail = self.search_config.fast_reward(node.state, action)
                fast_rewards.append(fast_reward)
                fast_reward_details.append(fast_reward_detail)
                # child = MCTSNode(state=None, action=action, parent=node,
                #                 fast_reward=fast_reward, fast_reward_details=fast_reward_details, calc_q=self.calc_q)
                # children.append(child)
            # fast_rewards_llm, fast_reward_details = self.search_config.normalize_llm_rewards(fast_reward_details)
            fast_rewards_llm, fast_reward_details = fast_rewards, fast_reward_details
            for action, fast_reward, fast_reward_detail in zip(actions, fast_rewards_llm, fast_reward_details):
                child = MCTSNode(state=None, action=action, parent=node,
                                 fast_reward=fast_reward, fast_reward_details=fast_reward_detail, calc_q=self.calc_q)
                children.append(child)
        # print(f"Time taken to expand with {'parallel' if self.parallel_actions else 'sequential'} actions: {time.time() - start}")
