...
def infer_single_sample(input_sample, causal_encoders, target_assignment, mean, std, device='cpu'):
    ...
    #     target_assignment[:, :1].repeat(1, 1),    # First group, no summation
    #     target_assignment[:, 1:2].repeat(1, 6),   # Second group, summed, now repeated
    #     target_assignment[:, 2:4].repeat(1, 1),   # Third group, no summation
    #     target_assignment[:, 4:5].repeat(1, 4),   # Fourth group, summed, now repeated
    #     target_assignment[:, 5:].repeat(1, 1)     # Fifth group, no summation
    # ], dim=1)
    input_sample = input_sample.to(device).float()
    for idx, model in enumerate(causal_encoders):
        model.eval()
        
        model_inputs = input_sample[:, target_assignment.T[idx].bool()].clone().detach()
